...
class ThumbWheel(KeyboardEntry):
...
#            checkkeywords(kw)
	...
	self.valueLabel.bind("<B1-Motion>", self.mouseMove)
        ...
        self.master.bind("<Button-3>", self.toggleOptPanel)

        KeyboardEntry.__init__(self, (self.master, self.canvas, self.valueLabel),
                               self.setFromEntry)
...
every time the widget value is set/modified"""
    ...
    def createCanvas(self, master, wheelPad=6):
        ...
        self.canvas = tkinter.Canvas(self.master, cd)
 ...
 #####################################################################
    ...
    def setLabel(self, labCfg):
        ...
        if not self.lab:
            self.lab = tkinter.Label(self.master, d)
