...
import tkinter, _py2k_string as string, math
...
class xyzGUI:
    ...
    def createEntries(self, master):
        ...
        self.thumbx = ThumbWheel(
            master=self.f, width=self.widthX, height=self.heightX, labcfg=self.labcfgX, wheelPad=self.wheelPadX
        )
        self.thumbx.callbacks.AddCallback(self.thumbx_cb)
        self.thumbx.master.grid(row=0, column=0)

        self.thumby = ThumbWheel(
            master=self.f, width=self.widthY, height=self.heightY, labcfg=self.labcfgY, wheelPad=self.wheelPadY
        )
        self.thumby.callbacks.AddCallback(self.thumby_cb)
        self.thumby.master.grid(row=1, column=0)

        self.thumbz = ThumbWheel(
            master=self.f, width=self.widthZ, height=self.heightZ, labcfg=self.labcfgZ, wheelPad=self.wheelPadZ
        )
        self.thumbz.callbacks.AddCallback(self.thumbz_cb)
        self.thumbz.master.grid(row=2, column=0)
    ...
    def configure(self, **kw):
        for key, value in list(kw.items()):
            ...
            if key == "labcfgX":
                self.setLabel(value, "x")
            elif key == "labcfgY":
                self.setLabel(value, "y")
            elif key == "labcfgZ":
                self.setLabel(value, "z")

            elif key == "continuousX":
                self.setContinuous(value, "x")
            elif key == "continuousY":
                self.setContinuous(value, "y")
            elif key == "continuousZ":
                self.setContinuous(value, "z")

            elif key == "precisionX":
                self.setPrecision(value, "x")
            elif key == "precisionY":
                self.setPrecision(value, "y")
            elif key == "precisionZ":
                self.setPrecision(value, "z")

            elif key == "typeX":
                self.setType(value, "x")
            elif key == "typeY":
                self.setType(value, "y")
            elif key == "typeZ":
                self.setType(value, "z")
                ...
                self.setOneTurn(value, "x")
            elif key == "oneTurnY":
                self.setOneTurn(value, "y")
            elif key == "oneTurnZ":
                self.setOneTurn(value, "z")

            elif key == "showLabelX":
                self.setShowLabel(value, "x")
            elif key == "showLabelY":
                self.setShowLabel(value, "y")
            elif key == "showLabelZ":
                self.setShowLabel(value, "z")

            elif key == "incrementX":
                self.setIncrement(value, "x")
            elif key == "incrementY":
                self.setIncrement(value, "y")
            elif key == "incrementZ":
                self.setIncrement(value, "z")
