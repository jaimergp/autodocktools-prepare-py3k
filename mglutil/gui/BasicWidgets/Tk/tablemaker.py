...
#
# $Header: /opt/cvs/python/packages/share1.5/mglutil/gui/BasicWidgets/Tk/tablemaker.py,v 1.8.10.1 2016/02/11 19:04:10 annao Exp $
#
# $Id: tablemaker.py,v 1.8.10.1 2016/02/11 19:04:10 annao Exp $
#
...
##          print 'new_alphas:', lut.alpha_arr
    ...
    def saveLUT(self, file=None):
        ...
        for lut in self.lut_list:
            ...
            alphas = numpy.take(lut.alpha_arr, alpha_inds, axis=0)
...
##              print 'in load_file: interval =', interval
    ...
    def getLookupTable(self):
        ...
        """
        rgba=numpy.zeros((self.xmaxval+1, 4), 'f')
        alpha = numpy.zeros(self.xmaxval+1,'i')
        for lut in self.lut_list:
            xminval = lut.xminval
            xmaxval = lut.xmaxval
...
##                       outline=UNSELECTED_COLOR)
    ...
    def drawIsoVal(self, isoVal, isoA, isoRGB):
        ...
        if self.continuous:
            self.callbacks['setalpha']([isoVal,
                            numpy.array([isoA]).astype(numpy.int16)])
            self.callbacks['setcolor']([isoVal, numpy.array([isoRGB],'f')])
    ...
    def removeIsoVal(self):
        ...
        for i , j in zip_longest(dots_id, lines_id):
        ...
        if self.continuous:
            self.callbacks['setalpha']([val,
                                numpy.array([alpha]).astype(numpy.int16)])
            self.callbacks['setcolor']([val, numpy.array([rgb],'f')])
    ...
    def flip_function(self):
        ...
        alpha_max= numpy.maximum.reduce(self.alpha_arr)
        self.alpha_arr = (self.alpha_arr*(-1)+alpha_max).astype(numpy.int16)
        shapes = [0,1]
...
##              print 'values=', self.values
    ...
    def update_yaxis(self, ymaxval):
        ...
        #self.alpha_arr = (self.alpha_arr*ymaxval/self.ymaxval).astype(numpy.int16)
        self.alpha_arr = numpy.clip(self.alpha_arr, 0, ymaxval).astype(numpy.int16)
        for i in range(1, len(self.points)-1):
    ...
    def compute_rgba(self, scalars, alphas):
        ...
        if d != 0 :
            ...
            s = numpy.arange(scalars[0], scalars[1])
            self.alpha_arr[sc0:sc1] =\
                (K*s+C).astype(numpy.int16)
            if scalars[1] > xminval or scalars[1] < xmaxval:
                self.color_arr[sc0:sc1] = \
                         numpy.transpose(numpy.array((Kr*s+Cr, Kg*s+Cg,
                                                    Kb*s+Cb)).astype('f'))
        ...
        if d != 0 :
            ...
            s = numpy.arange(scalars[1], scalars[2])
...
##              self.canvas.itemconfig('selected', fill=tkcolor)
    ...
    def doMoveIsoVal(self, x, y ,val, alpha):
        ...
        if oldval < val: #dot moved to the right
            d = val-oldval
            alphas = numpy.zeros(d+1).astype(numpy.int16)
            rgbs = numpy.zeros((d+1,3),'f')
            alphas[0] = oldalpha
            alphas[-1] = alpha
            rgbs[0] = oldrgb
            rgbs[-1] = rgb
...
##                                         last_scal+1-xminval]*1).astype('f')
            self.rgb = numpy.array(self.color_arr[first_scal-xminval:
                                       last_scal+1-xminval])
...
##              print 'scalars(in set_colors)=[%d,%d,%d]'%(scalar1,scalar2,scalar3)
            ...
            if d != 0:
                ...
                Cb = blue1-Kb*scalar1
                s = numpy.arange(scalar1, scalar2)
            ...
            if d != 0:
                ...
                Cb = blue2-Kb*scalar2
                s = numpy.arange(scalar2, scalar3)
...
##              print 'scalars(in set_colors)=[%d,%d,%d]'%(scalar1,scalar2,scalar3)
    ...
    def update_colors(self,scalar1, scalar2):
        ...
        d = scalar2-scalar1
        if d == 0: return [scalar1, scalar2, numpy.zeros((0,3),'f')]
        r = (red2-red1)/d
        g = (green2-green1)/d
        b = (blue2-blue1)/d
        s = numpy.arange(d)
        self.color_arr[scalar1:scalar2] = \
          numpy.transpose(numpy.array((r*s+red1,
