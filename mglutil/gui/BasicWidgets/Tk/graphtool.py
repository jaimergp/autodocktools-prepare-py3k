...
#
#
#$Id: graphtool.py,v 1.47.12.2 2016/05/12 22:43:32 mgltools Exp $
...
class GraphApp:
    ...
    def __init__(self,master=None,callback=None,continuous=1):
        ...
        self.d1scalewheel.master.pack(side="left")
        #tooltip
        self.balloon = Pmw.Balloon(f1)
        self.balloon.bind(self.d1scalewheel.master,"cutoff value for differences in Z xoordinates,small values generate more contours")
    ...
    def caluculate_ramp(self):
        ...
        for r in mynewpoints:
            ...
            if ra>=256:
                ra=255.0
            ramp.append(ra)
            dramp=numpy.array(ramp,'f')
        if len(dramp)!=0:
            return   dramp     
        else:
            dramp=numpy.arange(0,256,1,'f')
            return   dramp
...
every time the widget value is set/modified"""
    ...
    def resetAll_cb(self):
        ...
        if self.continuous or self.mousebuttonup:
            self.newd1ramp=numpy.arange(0,256,1,'f')
            self.callbacks.CallCallbacks(self.newd1ramp)
        #self.histvar.set(0)
        self.history=[]
