...
"""
    def __init__(self, master=None, width=150, height=30, max=None,
                 mode='percent', labelside='top', cb=None):
        ...
        self.callback = cb             # callback on click on bar
    ...
    def createWidget(self):
        ...
        cb = self.callback
        if cb:    
            self.canvas.tag_bind(self.backgroundBar, "<Button-1>", cb)
            self.canvas.tag_bind(self.progressLabel, "<Button-1>", cb)
...
##                 return
    ...
    def setColor(self, fgColor='blue', bgColor='darkblue'):
        self.canvas.itemconfig(self.progressBar, fill=fgColor)
        self.canvas.itemconfig(self.backgroundBar, fill=bgColor)
