...
class Tensor:
    ...
    def __mul__(self, other):
        if isTensor(other):
            ...
            return Tensor(numpy.inner(a, b), 1)
    ...
    def eigenvalues(self):
        ...
        if self.rank == 2:
            return numpy.linalg.eigvals(self.array)
        else:
    ...
    def diagonalization(self):
        ...
        if self.rank == 2:
            ev, vectors = numpy.linalg.eig(self.array)
            return ev, Tensor(numpy.transpose(vectors))
        else:
            raise ValueError('Undefined operation')
    ...
    def inverse(self):
        ...
        if self.rank == 2:
            return Tensor(numpy.linalg.inv(self.array))
        else:
            raise ValueError('Undefined operation')
