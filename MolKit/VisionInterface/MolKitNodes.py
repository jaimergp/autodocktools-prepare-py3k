...
from .MolKitTypes import msmsFound
...
class writePDBFiles(NetworkNode):
    """
    Write a PDB file for each set of coordinates in coordsList
...
def doit(self, mol, coordsList, filename):
    ...
    try:
        for i, coords in enumerate(coordsList):
            fname = filename+'''%05d'''%i+'.pdb'
            fnames.append(fname)
            allAtoms.coords = coords
            writer = PdbWriter()
            writer.write(fname, mol)
    finally:
    ...
    if mol:
        ...
        for i in range(0, len(rem290), 3):
            ...
            for j in range(3):
               ...
               mat[j] = (r1, r2, r3, t)
            mat[3] = (0., 0., 0., 1.)
            #matrices.append( numpy.array( numpy.transpose( mat) ) )
            matrices.append( mat )
...
class CenterAndRadius(NetworkNode):
    ...
    for mol in molecules:
        c = numpy.array(mol.chains.residues.atoms.coords)
        g = numpy.add.reduce(c) / len(c)
        centers.append( g )
        d = c-[g]
        radii.append( sqrt(max(numpy.add.reduce(d*d, 1))) )
...
class CalculateGasteigerCharges(NetworkNode):
    ...
    import numpy
    charge_total = numpy.add.reduce(mols.allAtoms.charge)
...
"""
    ...
    faces = f[:,:3]
    if component > 0:
        normals = (-1.*numpy.array( normals )).astype('f')
...
#        op.append(datatype='normals3D', name='vnormals')
    ...
    radii.shape = (-1, 1)
    coords = numpy.concatenate( (atoms.coords, radii), 1 )
    print coords.shape
