...
# $Header: /opt/cvs/python/packages/share1.5/AutoDockTools/autoanalyzeCommands.py,v 1.263.2.3 2016/02/11 09:24:07 annao Exp $
#
# $Id: autoanalyzeCommands.py,v 1.263.2.3 2016/02/11 09:24:07 annao Exp $
#
#
#
#
            ...
            if self.vf.userpref['VSPiInteractions_displayMode']['value'] == 'asCylindersAndCones':
                if len(pi_pi_list): #pi-pi
                    ...
                    for first,second in pi_pi_list:
                        center1 = (numpy.add.reduce(first.coords)/len(first)).tolist()
                        radii.append(0.7)
                        vertices.append(center1)
                        center2 = (numpy.add.reduce(second.coords)/len(second)).tolist()
                        radii.append(0.7)
    ...
    def getVertices(self, docking):
        ...
        if docking.flex_res:
            ...
            for c in docking.ch.conformations:
                center = numpy.add.reduce(numpy.array(c.getCoords()[:-ind])).astype('f')
                this_vert = (center/(num)).tolist()
                verts.append(this_vert)
        else:
            for c in docking.ch.conformations:
                center = numpy.add.reduce(numpy.array(c.getCoords())).astype('f')
                this_vert = (center/(num)).tolist()
                verts.append(this_vert)
        return verts
    ...
    def getEnergies(self, docking):
        ...
        for c in docking.ch.conformations:
            if hasattr(c, 'energy'):
                ...
                this_e = numpy.add.reduce(c.total_energies)
...
class ADShowBindingSite(MVCommand):
    ...
    def setupUndoBefore(self, docking):
        ...
        geomSet = macro_gca.get('ResidueLabels', [])
        ...
        if hasattr(docking, 'flex_res') and len(docking.flex_res):
            ...
            geomSet = lgca.get('ResidueLabels', [])
            kw = self.vf.labelByProperty.lastUsedValues['default']
            if len(geomSet):
                kw['only'] = 1
            else:
    ...
    def printInfo(self, event=None):
        ...
        if t=='hydrogen bonds':
            if not hasattr(self.docking.ligMol, 'vina_result') or self.docking.ligMol.vina_result==False:
    ...
    def buildLigandDisplay(self):
        ...
        geomset = lgc.atoms.get('sticks', [])
        if len(geomset)!=len(lig.allAtoms):
    ...
    def build(self, percentCutoff=1.):
        ...
        if hasattr(docking, 'flex_res') and len(docking.flex_res):
            ...
            #try to set the label fontScales here
            g = lig.geomContainer.geoms.get('ResidueLabels')
            if g:
                g.Set(fontScales=(.3,.3,.3))
        ...
        if self.showPiCation.get():
            ...
            if len(cation_pi):
                for first,second in cation_pi:
                    ...
                    if type(first)==list:
                        center1 = (numpy.add.reduce(AtomSet(first).coords)/len(first)).tolist()
                        radii.append(0.7)
                    else:
                        center1 = (first.coords)
                        radii.append(0.1)
                    if type(second)==list:
                        center2 = (numpy.add.reduce(AtomSet(second).coords)/len(second)).tolist()
                        radii.append(0.7)
                    else:
                        center2 = (second.coords)
                        radii.append(0.1)
        ...
        if hasattr(event, 'keysym') and event.keysym=='Right':
            ...
            for m in mols:
                ...
                if ind < nconf:
                    ...
                    #self.vf.labelByProperty(mols, properties = ['vina_energy'], font={'fontScales':(.5,.5,.5)})
                    g = m.geomContainer.geoms.get('ProteinLabels')
...
class ADDrawHistogram(MVCommand):
    ...
    def buildIt(self):
        ...
        if len(cList):
            ...
            #maxNumMembers = numpy.maximum.reduce(num_members)
            num_members = [len(x) for x in cList]
            maxNumMembers = max(num_members)
            num_membersSum = numpy.add.reduce(numpy.array(num_members))
...
#            item.getCoords()
    ...
    def updateColor(self, event=None):
        ...
        if confNum=='0':
            ...
            val_list = numpy.array(self.mol.allAtoms.estat_energy)
            mini = min(val_list)
            maxi = max(val_list)
    ...
    def Build_cb(self, event=None):
        ...
        if hasVDW and hasESTAT:
            total_energy = numpy.add(newMol.allAtoms.estat_energy, newMol.allAtoms.vdw_energy)
