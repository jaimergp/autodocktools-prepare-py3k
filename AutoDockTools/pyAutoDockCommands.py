...
#
# $Header: /opt/cvs/python/packages/share1.5/AutoDockTools/pyAutoDockCommands.py,v 1.21.8.1 2016/02/11 09:24:08 annao Exp $
#
# $Id: pyAutoDockCommands.py,v 1.21.8.1 2016/02/11 09:24:08 annao Exp $
#
#
#
#
...
from Pmv.stringSelectorGUI import SelectorGUI
...
second:second set of atoms
    ...
    def doit(self, first, second, display, cutoff, mat1, mat2):
        ...
        swap_dist = numpy.swapaxes(dist, 0,1)
...
weight: vdw weight to use
    ...
    def doit(self, first, second, weight):
        ...
        for i in range(len(first)):
            setattr(firstAts[i], self.prop, numpy.add.reduce(result[i]))
        #label each second atom by sum of its vdw interaction energies
        swap_result = numpy.swapaxes(result, 0,1)
        for i in range(len(swap_result)):
            setattr(secondAts[i], self.prop, numpy.add.reduce(swap_result[i]))
...
weight: hbonding weight to use
    ...
    def doit(self, first, second, weight):
        ...
        for i in range(len(first)):
            ...
            setattr(firstAts[i], 'sum_hb_energy', numpy.add.reduce(result[i]))
        #label each second atom by sum of its vdw interaction energies
        swap_result = numpy.swapaxes(result, 0,1)
        for i in range(len(swap_result)):
            ...
            setattr(secondAts[i], 'sum_hb_energy', numpy.add.reduce(swap_result[i]))
    ...
    def doit(self, first, second, weight):
        ...
        for i in range(len(first)):
            ...
            if a.element=='O':
                ...
                setattr(a, self.prop, numpy.add.reduce(result[i]))
        #label each second atom by sum of its vdw interaction energies
        swap_result = numpy.swapaxes(result, 0,1)
        #label each first atom by its desolvation interaction energies
        for i in range(len(swap_result)):
            ...
            if a.element=='O':
                ...
                setattr(a, self.prop, numpy.add.reduce(swap_result[i]))
            #setattr(secondAts[i], self.prop, numpy.add.reduce(swap_result[i]))
...
##         #label each first atom by sum of its vdw interaction energies
##         for i in range(len(first)):
##             #firstAts[i].vdw_energy = numpy.add.reduce(result[i])
##             hbond_val =  min(hbond_array[i])+max(hbond_array[i])
##             setattr(firstAts[i], self.prop, numpy.add.reduce(result[i])+hbond_val)
##         #label each second atom by sum of its vdw interaction energies
##         swap_result = numpy.swapaxes(result, 0,1)
##         swap_hbond_array = numpy.swapaxes(hbond_array, 0, 1)
##         for i in range(len(swap_result)):
##             #secondAts[i].vdw_energy = numpy.add.reduce(swap_result[i])
