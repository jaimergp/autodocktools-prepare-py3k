...
#
# $Id: cluster.py,v 1.41.4.1 2016/02/11 09:24:08 annao Exp $
#
"""
Some definitions:
    ...
    def __init__(self, data, sort='binding'):
        ...
        if sort == 'docking':
            ...
            if hasattr(data[0], 'subset'):
                ...
                raise Exception("no subsets have been set up for conformations!")
            for d in data:
        ...
        self.dist_matrix = numpy.zeros([len(data), len(data)]) - 1.0
...
1.5 2.0 binding
  ...
  1   1   1   1  2.548  1.014  1.014 -14.210
    ...
    def rebuild_clusters(self, clusterLists, tolerance):
        ...
        for (cx, cl) in zip_longest(range(len(clusters)),
                                 clusters):
            for (rank, conf) in zip_longest(range(len(cl)), cl):
                conf.cluster_dict[tolerance] = (cx, rank)
