...
# $Header: /opt/cvs/python/packages/share1.5/AutoDockTools/autogpfCommands.py,v 1.132.2.4 2017/05/01 22:42:25 forli Exp $
#
# $Id: autogpfCommands.py,v 1.132.2.4 2017/05/01 22:42:25 forli Exp $
#
#
#
#
    ...
    def updateValues(self):
        ...
        if self.gridcenter == 'auto':
            if hasattr(self.vf.gpo, 'receptor'):
                if not hasattr(self.vf.gpo.receptor, 'center'):
                    self.vf.gpo.receptor.getCenter()
...
class Gpf4MacroInit(MVCommand):
    ...
    def doit(self, mol, **kw):
        ...
        mol.gpf_init = 1
        self.vf.gpo.receptor = mol
        #self.vf.gpo.set_receptor4(mol.parser.filename, receptor_type_str)
        self.vf.gpo.set_receptor4(filename, receptor_type_str)
        # SF: the lines below will raise an exception all the time (unless the file is in the current path)
        #     uncommented line above and commented two before to restore original behavior.
        #mol_name = os.path.basename(os.path.splitext(mol.parser.filename)[0]) + '.pdbqt'
        #self.vf.gpo.set_receptor4(mol_name, receptor_type_str)
        if not hasattr(mol, 'center'):
            mol.getCenter()
        if self.vf.hasGui:
            self.vf.colorByAtomType(mol, ['lines'], topCommand=0, redraw=1)
...
class Gpf4MacroReader(MVCommand):
    ...
    def guiCallback(self):
        """called each time the 'read macromolecule' button is pressed"""
        macroFile = self.vf.askFileOpen(types=[('PDBQT files', '*.pdbqt'),('all files', '*.*')], 
            title = 'Macromolecule File:')
        #allow user to set typeCharges and addSolPar here
        #if macroFile is not None:
        if macroFile:
